...
export function CustAvatarGroup( props : any ) {
  ...
  return (
    <AvatarGroup >
      ...
      <Avatar size="lg">
        <AvatarImage src={props?.data[1]?.snippet?.thumbnails?.default.url ?? "N"} alt={props?.data[1]?.snippet?.title} />
        <AvatarFallback>{props?.data[1]?.snippet?.title[0]}</AvatarFallback>
      </Avatar >
      <Avatar size="lg">
     <AvatarImage src={props?.data[2]?.snippet?.thumbnails?.default.url ?? "N" }  alt={props?.data[2]?.snippet?.title} />
