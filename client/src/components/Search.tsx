...
function SelectedSymptopms(props: { selected: string[], setSelected: React.Dispatch<React.SetStateAction<string[]>> }) {
  ...
  const showSelected = (block: string, index: number) => <div className="flex max-w-xs items-center gap-1.5 bg-amber-100 text-amber-900 px-3 py-1 rounded-full text-sm font-medium border border-amber-200 animate-in fade-in zoom-in duration-200" key={block}> <p className="truncate" > {block} </p>  <button onClick={() => removing(block)} className="bg-transparent text-amber-600 text-lg leading-none"> x </button></div>;
...
function SymptomList(props: { filteredlist: string[], alrdselected: string[], setSelected: React.Dispatch<React.SetStateAction<string[]>>, setQuery : React.Dispatch<React.SetStateAction<string>>}) {
  const dispatch = useDispatch();
  ...
  return (
    <div 
    className="absolute flex flex-col items-center bg-white z-50 w-full mt-2 pb-1 rounded-xl shadow-xl overflow-hidden animate-in slide-in-from-top-2 duration-200"
    >
      {props.filteredlist.map((data, index) => <a className="block w-full text- px-5 py-3 text-sm font-medium text-slate-700 hover:bg-amber-50 hover:text-amber-900 transition-colors border-b border-slate-50 last:border-none cursor-pointer"
       onClick={() => {Adding(data); props.setQuery("")} } key={data}> {data}</a>)}
    </div>
  )
}
